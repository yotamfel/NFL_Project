...
import { MOCK_PLAYER_PROFILE } from '../data/mock'
...
export default function PlayerProfile() {
  const { id } = useParams()
  // Stage 8: replace with GET /api/players/${id}
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          {draft && (
            <div className="text-right shrink-0">
              ...
              <p className="text-slate-400 text-sm">{draft.team} · {draft.college}</p>
          ...
          <div className="mt-5 pt-5 border-t border-slate-700">
            ...
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <div key={label} className="bg-slate-900 rounded-xl px-4 py-2.5 text-center min-w-20">
      ...
      {/* Category sections */}
      {categories.map(cat => {
        const cols = COLS_BY_CATEGORY[cat.category]
        const chartLines = CHART_LINES_BY_CATEGORY[cat.category]
        if (!cols) return null
        return (
          <div key={cat.category} className="bg-slate-800 rounded-2xl p-5 space-y-4">
            ...
            {chartLines && cat.seasons.length > 1 && (
              <CareerLineChart data={cat.seasons} xKey="season" lines={chartLines} />
            )}
